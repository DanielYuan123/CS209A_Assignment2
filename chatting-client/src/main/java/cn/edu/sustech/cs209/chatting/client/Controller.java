...
public class Controller implements Initializable {
  ...
  private void showRegisterWindow() {
    ...
    userNameLabel.setLayoutX(144.0);
    ...
    registerBtn.setOnMouseClicked(event -> {
      ...
      if (!pwd.equals(passwordCheckTextField.getText())) {
  ...
  private boolean registerServer() {
    ...
    if (response.equals("OK")) {
      return true;
    } else return !response.equals("DENY");
  }
  ...
  private void sendFile() {
    ...
    File file = fc.showOpenDialog(chatList.getScene().getWindow());
    if (file != null) {
      try {
        byte[] fileByte = Files.readAllBytes(file.toPath());
        String encoded = Base64.getEncoder().encodeToString(fileByte);

        if (userGroupSet.containsKey(currentChatMate)) {
          try {
            ArrayList<String> usersSendTo = userGroupSet.get(currentChatMate);

            Message message = new Message(System.currentTimeMillis(), username, currentChatMate, encoded);
        ...
        /////
//                FileChooser fc = new FileChooser();
//                fc.setTitle("receive file");
//                File file1 = fc.showSaveDialog(chatList.getScene().getWindow());
//                if(file1!=null){
//                    byte[] decoder = Base64.getDecoder().decode("s");
//                    Files.write(file1.toPath(),decoder);
//                }
      } catch (IOException e) {
        throw new RuntimeException(e);
      }
        ...
        while (!Thread.currentThread().isInterrupted()) {
          ...
          if (receivedMessage.isGroupMessage()) {
            ...
            if (!userGroupSet.containsKey(to)) {
              ...
              usersDialogSet.put(to, new ArrayList<>());
            }
            if (currentChatMate != null) {
          ...
          if (!chatList.getItems().contains(from)) {
            ...
            usersDialogSet.put(from, new ArrayList<>());
          }
          if (currentChatMate != null) {
  ...
  private static class UserCellFactory implements Callback<ListView<String>, ListCell<String>> {
    ...
    double blue = rd.nextInt(255);
    double green = rd.nextInt(255);
    double redi = (red + 125) % 255;
    double bluei = (blue + 125) % 255;
    double greeni = (green + 125) % 255;
      ...
      return new ListCell<String>() {
        ...
        public void updateItem(String msg, boolean empty) {
          ...
          if (empty || Objects.isNull(msg)) {
            ...
            Circle nameProfile = new Circle();


            nameProfile.setRadius(8);
            nameProfile.setFill(new Color(red / 255, blue / 255, green / 255, 1.0));
          ...
          if (msg.getIsFile()) {
            ...
            downloadBtn.setOnMouseClicked(new EventHandler<MouseEvent>() {
              ...
              public void handle(MouseEvent event) {
                ...
                File file1 = fc.showSaveDialog(chatList.getScene().getWindow());
                if (file1 != null) {
                  byte[] decoder = Base64.getDecoder().decode(msg.getData());
                  try {
                    Files.write(file1.toPath(), decoder);
                  } catch (IOException e) {
                    throw new RuntimeException(e);
            ...
            if (username.equals(msg.getSentBy())) {
              ...
              wrapper.getChildren().addAll(downloadBtn, msgLabel, nameLabel, timeLabel);
              msgLabel.setPadding(new Insets(0, 20, 0, 0));
            } else {
              wrapper.setAlignment(Pos.TOP_LEFT);
              wrapper.getChildren().addAll(timeLabel, nameLabel, msgLabel, downloadBtn);
